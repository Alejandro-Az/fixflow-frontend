...
- Export mantenimiento a calendario (ICS recurrente): `POST /devices/{device_id}/exports/maintenance/calendar`.
- Legal consents: `GET /auth/legal-consents/latest`, `POST /auth/legal-consents/accept`.
...
### Consentimiento legal (Términos y Privacidad)
- **Consultar estado y versión vigente:** `GET /auth/legal-consents/latest`
  - Responde con `data.required` (versión vigente en backend) y `data.accepted` (última aceptación del usuario o `null`).
- **Registrar aceptación:** `POST /auth/legal-consents/accept`
  - Body mínimo:
    - `terms_accepted: true`
    - `privacy_accepted: true`
    - `terms_version` y `privacy_version` (usar exactamente las versiones de `data.required`)
    - `source` opcional (`web|mobile|android|ios`)

Regla de frontend:
1. Antes de finalizar onboarding, llamar `GET /auth/legal-consents/latest`.
2. Si `data.accepted` es `null` o la versión no coincide con `data.required`, mostrar modal/pantalla obligatoria.
3. Solo permitir continuar cuando `POST /auth/legal-consents/accept` responda `201`.

Errores esperados:
- `422 VALIDATION_ERROR` si las versiones no coinciden o si no se marcan ambos checks.
- `401 AUTH_UNAUTHENTICATED` sin token válido.
...
### Exportar recordatorio semestral a calendario (sin OAuth)
- Endpoint: `POST /devices/{device_id}/exports/maintenance/calendar`
- Objetivo: generar un archivo `.ics` importable en Google Calendar y otros calendarios.
- El usuario define `start_date` (día inicial). Backend genera recurrencia cada 6 meses desde esa fecha.
...
Respuesta:
- `data.content_base64` (archivo `.ics` codificado)
- `data.file_name`
- `data.mime_type = text/calendar; charset=UTF-8`

Implementación frontend:
1. Decodificar `content_base64`.
2. Guardar archivo `.ics` temporal.
3. Lanzar compartir/abrir para que el usuario lo agregue a su calendario.
